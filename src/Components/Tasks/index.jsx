            ...
            {tasks.map((task, i) => (
                <div key={task.id} ref={taskRef} className="group flex flex-row items-center justify-between px-4 py-2 border-gray-200 border-t">
                    ...
                    <input type="text"
                        ...
                        value={task.text}
                        aria-label={task.text} aria-required="true"
                        onChange={(e) => { updateTask(noteId, task.id, e.target.value, task.isCompleted) }} />
                    <button onClick={() => removeTask(noteId, task.id)} aria-label="Remove Task" aria-required="true" tabIndex="true" className="opacity-0 group-hover:opacity-100 focus:opacity-100 focus:outline-none">
